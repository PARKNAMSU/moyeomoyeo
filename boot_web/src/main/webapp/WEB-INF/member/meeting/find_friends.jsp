...
<script>
	...
	function followMember(email,t,type){
		var el = $(t);
		console.log(el)
		console.log(email)
		$.ajax({
			...
			if(type === 'recommend'){
				var el = t.parentNode.parentNode.parentNode;
				$(el).remove()
				return false;
			}
			el.removeAttr("onclick");
			el.attr("onclick","unfollowMember('"+email+"',this)")
			el.text("언팔로우")
		})
	}
	function unfollowMember(email,t){
		...
		$.ajax({
			...
			console.log("uf")
			el.removeAttr("onclick");
			el.attr("onclick","followMember('"+email+"',this)")
			el.text("팔로우")
		})
	}
		...
		$.ajax({
			...
			for(var i = 0; i < data_obj.length; i++){
				...
				var friends_el = '	<div class="friends_div_01_02">'
					...
					+'<div class="fl" style="width: 270px; height: 100%;">'
						+'<div style="text-align: right; width: 100%;">'
						+'<a class="a_btn" onclick="followMember(\''+data_obj[i].email+'\',this,\'recommend\')">팔로우</a>'
						+'</div>'
					+'<div>'
					+	'<p>'+data_obj[i].name+'('+data_obj[i].nick_name+')'+'</p>'
					+'</div>'
